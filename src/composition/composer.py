...
class VideoComposer:
    ...
    def compose_with_audio_overlay(
        ...
        """Combine a video with an audio track using professional mixing.

        Features:
        - YouTube-optimized loudness normalization (target -14 LUFS)
        - Voiceover EQ boost (2-4kHz clarity)
        - Background music with auto-ducking (lowers when voice is active)
        - AAC 256kbps for YouTube quality
        ...
        Args:
            ...
            normalize_loudness: Whether to normalize to YouTube LUFS target
        ...
        if background_music and background_music.exists():
            ...
            # Professional audio mixing with ducking and EQ
            # Voice: EQ boost 2-4kHz for clarity, compress for consistent level
            # Music: auto-duck below voice, low-pass to stay out of voice range
            voice_filter = (
            ...
            music_duck_filter = (
                ...
                "lowpass=f=8000,"
                "acompressor=threshold=-25dB:ratio=4:attack=10:release=200"
            )

            filter_complex = (
                f"[1:a]{voice_filter}[voice];"
                f"[2:a]{music_duck_filter}[music];"
                "[voice][music]amix=inputs=2:duration=first:dropout_transition=2,"
                "sidechaincompress=threshold=-30dB:ratio=6:attack=5:release=100[mixed]"
            )

            if normalize_loudness:
                # YouTube target: -14 LUFS integrated, -1 dBTP true peak
                filter_complex = (
                    f"[1:a]{voice_filter}[voice];"
                    f"[2:a]{music_duck_filter}[music];"
                    "[voice][music]amix=inputs=2:duration=first:dropout_transition=2[premix];"
                    "[premix]loudnorm=I=-14:LRA=11:TP=-1[mixed]"
                )
            ...
            # Voice-only: EQ + compression + loudness normalization
            if normalize_loudness:
                voice_filter = (
                    "acompressor=threshold=-20dB:ratio=3:attack=5:release=50,"
                    "equalizer=f=3000:t=q:w=1.5:g=3,"
                ...
                cmd.extend([
                    "-filter_complex", f"[1:a]{voice_filter}[aout]",
                    "-map", "0:v:0",
                    "-map", "[aout]",
                ])
            else:
        ...
        cmd.extend([
            "-c:v", "copy",
            "-c:a", "aac",
            "-b:a", "256k",        # YouTube recommended: 256kbps for stereo
            "-ar", "48000",         # YouTube recommended: 48kHz sample rate
            "-ac", "2",             # Stereo output
