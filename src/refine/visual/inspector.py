...
# Remotion URLs - loaded from config at runtime
def _get_remotion_urls():
    from ...config import load_config
    base = load_config().remotion.dev_url
    return base, f"{base}/SingleScenePlayer"

REMOTION_BASE_URL = None  # Use _get_remotion_urls() at runtime
SINGLE_SCENE_PLAYER_URL = None  # Use _get_remotion_urls() at runtime
...
class ClaudeCodeVisualInspector:
    ...
    def refine_scene(self, scene_index: int) -> SceneRefinementResult:
        ...
        # Architectural fix #4 — engagement scoring pass.
        # Runs AFTER the LLM review so we get a numeric gate on top of the
        # qualitative feedback. Failures become ENGAGEMENT-typed Issues so
        # the applicator can regenerate, not just tweak pixels.
        try:
            engagement = self._score_engagement(
                scene_info, scene_index, scene_file
            )
        except Exception as exc:  # scoring must never break the pipeline
            self._log(f"   ⚠ Engagement scoring skipped: {exc}")
            engagement = None
        if engagement is not None:
            result.engagement_score = engagement.to_dict()
            for issue_text in engagement.issues:
                result.issues_found.append(
                    Issue(
                        beat_index=0,
                        principle_violated=IssueType.ENGAGEMENT,
                        description=issue_text,
                        severity="high" if "dead_air" in issue_text else "medium",
                    )
                )
            if not engagement.passes():
                result.verification_passed = False
                # Architectural enforcement: engagement failure triggers a
                # bounded auto-regenerate. The generator's own validator
                # still has to approve the new .tsx; this just kicks off
                # the retry instead of relying on a human to notice.
                self._trigger_auto_regenerate(
                    scene_info=scene_info,
                    scene_file=scene_file,
                    reason=f"engagement_below_threshold: {engagement.issues}",
                )
    ...
    def _trigger_auto_regenerate(
        ...
        *,
        ...
        """Auto-regenerate a scene that failed the engagement gate.

        Bounded retry (default 1) — we don't want a quality gate to turn
        into an infinite loop. Returns True if a fresh .tsx was written.
        """
        try:
            from ...scenes.scene_generator import regenerate_scene
        except Exception as exc:  # pragma: no cover - defensive
            self._log(f"   ⚠ Auto-regenerate unavailable: {exc}")
            return False

        script_path = self.project.root_dir / "script.json"
        if not script_path.exists():
            self._log(
                f"   ⚠ Auto-regenerate skipped: {script_path} not found"
            )
            return False

        scene_id = scene_info.get("id") or scene_info.get("scene_id")
        if not scene_id:
            return False

        scenes_dir = scene_file.parent
        self._log(
            f"   ♻  Auto-regenerating '{scene_id}' — {reason}"
        )
        for attempt in range(max_attempts):
            try:
                if regenerate_scene(script_path, scene_id, scenes_dir):
                    self._log(f"   ♻  Regenerated '{scene_id}' on attempt {attempt + 1}")
                    return True
            except Exception as exc:
                self._log(f"   ⚠ Regenerate attempt {attempt + 1} failed: {exc}")
        self._log(f"   ⚠ Auto-regenerate exhausted for '{scene_id}'")
        return False
    ...
    def _score_engagement(
        ...
        """Capture uniformly-sampled frames and score engagement.

        Returns None if Playwright isn't available or no frames could be
        captured — scoring is best-effort, never a hard dependency.
        """
        if not PLAYWRIGHT_AVAILABLE:
            return None

        duration = float(scene_info.get("duration_seconds", 30))
        out_dir = self.project.root_dir / "refinement" / "engagement" / scene_info["id"]
        out_dir.mkdir(parents=True, exist_ok=True)

        samples = sample_frame_paths(out_dir, duration, interval_seconds=1.5)

        # Capture screenshots via the existing ScreenshotCapture helper.
        try:
        ...
        try:
            ...
            for path, t in samples:
                ...
                try:
                    ...
                    if getattr(shot, "path", None) and Path(shot.path).exists():
                        captured.append(Path(shot.path))
                        times.append(t)
                except Exception:
                    continue
        finally:
        ...
        score = score_scene_engagement(captured, times)
        self._log(
            f"   📊 Engagement: density={score.content_density:.2f}, "
            f"interrupts/10s={score.pattern_interrupt_rate:.2f}, "
            f"dead_air_spans={len(score.dead_air_spans)}"
        )
        return score
    ...
    def _ensure_remotion_running(self) -> bool:
        ...
        if check_remotion_running(_get_remotion_urls()[0]):
...
class VisualInspector:
    ...
    def __init__(
        ...
        if llm_provider is None:
            from ...config import load_config
            config = LLMConfig(provider="claude-code", model=load_config().llm.model)
