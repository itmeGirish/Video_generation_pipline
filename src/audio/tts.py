...
class ElevenLabsTTS(TTSProvider):
    ...
    def __init__(self, config: TTSConfig, api_key: str | None = None):
        ...
        self.voice_id = config.voice_id or config.elevenlabs.default_voice_id
        self._voice_settings = {
            "stability": config.elevenlabs.stability,
            "similarity_boost": config.elevenlabs.similarity_boost,
        }
        self._cost_per_char = config.elevenlabs.cost_per_character
    ...
    def generate(self, text: str, output_path: str | Path) -> Path:
        ...
        with httpx.Client(timeout=self.config.elevenlabs.request_timeout) as client:
    ...
    def generate_stream(self, text: str) -> Iterator[bytes]:
        ...
        with httpx.Client(timeout=self.config.elevenlabs.stream_timeout) as client:
            with client.stream(
                "POST",
                url,
                headers=self._get_headers(),
    ...
    def get_available_voices(self) -> list[dict]:
        ...
        with httpx.Client(timeout=self.config.elevenlabs.list_voices_timeout) as client:
    ...
    def generate_with_timestamps(
        ...
        with httpx.Client(timeout=self.config.elevenlabs.timestamps_timeout) as client:
            response = client.post(
                url,
                headers=self._get_headers(),
                json=payload,
    ...
    def estimate_cost(self, text: str) -> float:
        ...
        """
        return len(text) * self._cost_per_char
...
class MockTTS(TTSProvider):
    ...
    def __init__(self, config: TTSConfig):
        ...
        self._wpm = speech.words_per_minute
        self._inter_word_gap = speech.inter_word_gap
        self._word_dur_base = speech.word_duration_base
        self._word_dur_factor = speech.word_duration_length_factor
        self._word_len_ref = speech.word_length_reference
    ...
    def generate(self, text: str, output_path: str | Path) -> Path:
        ...
        duration_seconds = max(1.0, (words / self._wpm) * 60)
    ...
    def generate_with_timestamps(
        ...
        words = text.split()
        duration_seconds = max(1.0, (len(words) / self._wpm) * 60)
        ...
        for word in words:
            ...
            if clean_word:
                ...
                word_duration = avg_word_duration * (self._word_dur_base + self._word_dur_factor * len(clean_word) / self._word_len_ref)
                word_timestamps.append(
                    WordTimestamp(
                        word=clean_word,
                ...
                current_time += word_duration + self._inter_word_gap
