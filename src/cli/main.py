...
def cmd_voiceover(args: argparse.Namespace) -> int:
    ...
    if provider_name == "manual":
        ...
        # Create TTS config — pull provider, voice_id, AND model from project config
        config = Config()
        config.tts.provider = provider_name
        if project.tts.voice_id:
            config.tts.voice_id = project.tts.voice_id
        if hasattr(project.tts, "model") and project.tts.model:
            config.tts.model = project.tts.model
...
def cmd_storyboard(args: argparse.Namespace) -> int:
    ...
    # Load voiceover manifest if it exists (for audio durations + word timestamps for captions)
    voiceover_data = {}
    if voiceover_manifest.exists():
        with open(voiceover_manifest, encoding="utf-8") as f:
    ...
    # Build storyboard
    # Auto-detect background music file in project/music/ directory
    music_dir = project.root_dir / "music"
    background_music_config = None
    if music_dir.exists():
        # Find first mp3/wav in music dir
        music_files = sorted(list(music_dir.glob("*.mp3")) + list(music_dir.glob("*.wav")))
        if music_files:
            # Path should be relative to voiceover for remotion staticFile
    ...
    for scene in narrations.get("scenes", []):
        ...
        # Use scene_id directly as the registry key — matches the generated
        # scene components in projects/{project}/scenes/{scene_id}.tsx
        scene_type = f"{project.id}/{scene_id}"
        ...
        # Include word timestamps for captions (if available)
        word_timestamps = []
        if scene_id in voiceover_data:
            word_timestamps = voiceover_data[scene_id].get("word_timestamps", [])
    ...
    # Scene components are generated per-project in projects/{id}/scenes/
    # No visual block specs needed — each scene has its own .tsx component
...
def cmd_scenes(args: argparse.Namespace) -> int:
    ...
    Each scene's visual_description drives what gets generated.
    Uses claude --print to generate .tsx components per scene.
    """
    from ..project import load_project
    from ..scenes.scene_generator import generate_all_scenes, regenerate_scene
    ...
    if getattr(args, "scene", None):
        scene_id = args.scene
        print(f"Regenerating scene: {scene_id}")
        ok = regenerate_scene(script_path, scene_id, scenes_dir, timeout=getattr(args, "timeout", 300))
        return 0 if ok else 1

    # Check if scenes already exist
    if scenes_dir.exists() and list(scenes_dir.glob("*.tsx")) and not args.force:
        existing = [f.stem for f in scenes_dir.glob("*.tsx") if f.stem != "index"]
        print(f"Scenes already exist ({len(existing)} files) in {scenes_dir}")
        print("Use --force to regenerate.")
        return 0
    ...
    try:
        ...
        if results["failed"] > 0:
            print(f"\n⚠ {results['failed']} scenes failed. Regenerate individually:")
            for sid in results["scene_ids"]:
                tsx = scenes_dir / f"{sid}.tsx"
                if not tsx.exists() or tsx.stat().st_size < 100:
                    print(f"  python -m src.cli scenes {args.project} --scene {sid} --force")
            return 1
...
def _cmd_scenes_verify(args: argparse.Namespace, project) -> int:
    ...
    if result.success:
        print("[OK] All scenes pass syntax verification")
        if result.fixed_files:
            print(f"\nAuto-fixed files: {', '.join(result.fixed_files)}")
        return 0

    # Report errors
    print(f"[FAIL] Found {result.error_count} syntax error(s)")
    print()
...
def _cmd_scenes_sync(args: argparse.Namespace, project) -> int:
    """Sync scene timing — regenerate scenes with updated timestamps."""
    from ..scenes.scene_generator import generate_all_scenes

    # Sync = regenerate scenes with --force to pick up new timestamps
    print(f"Syncing scenes for {project.id} (regenerating with current timestamps)")
    script_path = project.root_dir / "script" / "script.json"
    scenes_dir = project.root_dir / "scenes"
    ...
    try:
        results = generate_all_scenes(script_path, scenes_dir)
        print(f"Synced {results['succeeded']}/{results['total']} scenes")
...
def _cmd_scenes_regenerate_single(args: argparse.Namespace, project) -> int:
    """Regenerate a single scene by scene_id."""
    from ..scenes.scene_generator import regenerate_scene
    ...
    print(f"Regenerating scene: {scene_id}")
    ok = regenerate_scene(script_path, scene_id, scenes_dir)
    return 0 if ok else 1
...
def cmd_generate(args: argparse.Namespace) -> int:
    ...
    def step_output_exists(step: str) -> bool:
        ...
        if step == "plan":
            ...
            if not scenes_dir.exists():
                return False
            tsx_files = [f for f in scenes_dir.glob("*.tsx") if f.name not in ("index.tsx",)]
            # Check against script — must have same number of scene files as script defines
            script_path = project.root_dir / "script" / "script.json"
            if script_path.exists():
                try:
                    with open(script_path, encoding="utf-8") as f:
    ...
    for step_num, step in enumerate(steps_to_run, 1):
        ...
        if not args.force and step_output_exists(step):
            print(f"[OK] Output already exists, skipping (use --force to regenerate)")
            continue
        ...
        if step == "plan":
            ...
            step_args.mock = args.mock
            step_args.timeout = args.timeout
            # Always regenerate storyboard when it's the starting step (needs fresh visual specs)
            step_args.force = args.force or not step_output_exists(step) or step == start_step
            step_args.verbose = False
            result = cmd_storyboard(step_args)

        elif step == "render":
            # Auto-delete old video when re-rendering
            old_video = project.get_path("final_video")
            if old_video.exists():
        ...
        print(f"[OK] {step.upper()} completed successfully")
    ...
    if mp4_files:
        ...
        # Run YouTube quality check
        try:
            from ..quality import check_video_quality
            print(f"\n{'-' * 60}")
            print("YOUTUBE QUALITY CHECK")
            print(f"{'-' * 60}")
            report = check_video_quality(latest_video)
            print(f"Score: {report.score}/100 {'[PASS]' if report.passed else '[NEEDS WORK]'}")
            for issue in report.issues:
                icon = {"error": "[!]", "warning": "[~]", "info": "[i]"}.get(issue.severity, "[ ]")
                print(f"  {icon} {issue.message}")
            if not report.issues:
                print("  No issues found - video is YouTube-ready!")
            if "loudness_lufs" in report.metrics:
                print(f"  Loudness: {report.metrics['loudness_lufs']:.1f} LUFS")
        except Exception as e:
