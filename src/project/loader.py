...
class TTSConfig:
    ...
    voice_id: str = ""  # Set via config or ELEVENLABS_DEFAULT_VOICE_ID env var
    model: str = ""  # Optional: overrides the default model (e.g., "eleven_turbo_v2_5")
    output_format: str = ""  # Optional: overrides default output format
...
def load_project(project_path: str | Path) -> Project:
    ...
    if "model" in tts_data:
        tts_kwargs["model"] = tts_data["model"]
    if "output_format" in tts_data:
        tts_kwargs["output_format"] = tts_data["output_format"]
    tts_config = TTSConfig(**tts_kwargs)
