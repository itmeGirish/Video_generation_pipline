...
class StoryboardGenerator:
    ...
    def _generate_scene_beats(
        ...
        word_ts_lines = []
        # Include all word timestamps for precise sync (key words + timing boundaries)
        # For very long scenes, include every 2nd word to stay under token limits
        timestamps = tts_result.word_timestamps
        if len(timestamps) > 150:
            # Keep first 30, last 30, and every 3rd word in the middle
            timestamps = timestamps[:30] + timestamps[30:-30:3] + timestamps[-30:]
