...
class ClaudeCodeLLMProvider(LLMProvider):
    ...
    def generate(self, prompt: str, system_prompt: str | None = None) -> str:
        ...
        """
        result = self._run_cmd(prompt, system_prompt, tools=[])
    ...
    def generate_json(
        ...
        result = self._run_cmd(json_prompt, system_prompt, tools=[])
    ...
    def generate_with_file_access(
        ...
        cmd, temp_file = self._build_command(prompt, system_prompt, tools=tools)
    ...
    def _build_command(
        ...
        For long prompts (>4000 chars), writes prompt to a temp file and
        uses stdin piping to avoid Windows command-line length limits.
        ...
        Returns:
            Tuple of (command as list of strings, temp file path or None)
        """
        import tempfile

        temp_file = None

        # On Windows, command lines are limited (~8191 chars).
        # For long prompts, wrap the prompt so the total command stays under limit.
        # Calculate approximate command length (prompt + system_prompt + flags)
        system_len = len(system_prompt) if system_prompt else 0
        total_len = len(prompt) + system_len + 200  # 200 for flags/model

        if total_len > 7000:
            # Write prompt to temp file with instruction to read it
            temp_file = Path(tempfile.mktemp(suffix=".md", prefix="claude_prompt_"))
            temp_file.write_text(prompt, encoding="utf-8")
            # Tell claude to read the prompt from the file
            short_prompt = f"Read the file {temp_file} and follow the instructions in it exactly. Return your response directly."
            cmd = ["claude", "--print", short_prompt]
        else:
        ...
        if system_prompt:
            # If system prompt is also very long, write it to a file too
            if len(system_prompt) > 3000:
                sys_file = Path(tempfile.mktemp(suffix=".md", prefix="claude_system_"))
                sys_file.write_text(system_prompt, encoding="utf-8")
                cmd.extend(["--system-prompt", f"Read and follow the system instructions in {sys_file}"])
            else:
                cmd.extend(["--system-prompt", system_prompt])

        # If we used temp files, we need Read tool access
    ...
    def _run_cmd(
        ...
        """Build and run a claude CLI command, handling long prompts via temp files.
        ...
        # Check if system prompt also created a temp file
        import glob as glob_mod
        for f in Path(temp_file.parent if temp_file else "/tmp").glob("claude_system_*.md"):
            temp_files.append(f)
    ...
    def _parse_json_response(self, response: str) -> dict[str, Any]:
        ...
        if not response:
            raise ClaudeCodeError(
                "Claude Code returned empty output. This usually means the "
                "subprocess stdout was captured with a non-utf-8 encoding "
                "and the reader thread died — check encoding='utf-8' on "
