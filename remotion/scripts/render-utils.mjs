...
export function validateSceneTypes(storyboard, projectScenesDir, fs, path) {
  ...
  // Extract registry keys using regex - matches SCENE_REGISTRY or PROJECT_SCENES
  const registryMatch = indexContent.match(/(?:SCENE_REGISTRY|PROJECT_SCENES)[^{]*\{([^}]+)\}/s);
  if (!registryMatch) {
    return {
      valid: false,
      error: "Could not parse SCENE_REGISTRY or PROJECT_SCENES from index.ts",
      mismatches: [],
    };
  }

  const registryBlock = registryMatch[1];
  const keyPattern = /^\s*"?(\w+)"?\s*:\s*\w+,?/gm;
