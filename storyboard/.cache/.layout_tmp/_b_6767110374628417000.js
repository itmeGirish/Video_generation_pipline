...
const __mainEl = _scnCard({ borderColor: D.amber, fadeIn,
  children: [
    _scnTitle('USE THIS FOR: Q1 budget planning', D.amber, Math.round(width * 0.022)),
    _scnLine("IF you're an OS company: 70% OpenAI / 30% Anthropic", D.violet),
    _scnLine("IF you're a precision-services company: 70% Anthropic / 30% OpenAI", D.cyan),
    _scnLine("IF you're a SaaS startup: 50/50 with routing layer", D.amber),
    _scnSubtitle('REVISIT QUARTERLY — both ship every 6–8 weeks', D.text_dim),
  ]
});
