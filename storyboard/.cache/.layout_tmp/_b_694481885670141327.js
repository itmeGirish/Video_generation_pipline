...
const __mainEl = _scnCard({ borderColor: D.green, fadeIn,
  children: [
    _scnTitle('PIPELINE TOTAL', D.green, Math.round(width * 0.024)),
    React.createElement('div', { style:{
      display:'flex', alignItems:'center', justifyContent:'center', gap: 32,
      fontFamily: D.font_display, fontWeight: 900, fontSize: Math.round(width * 0.080), color: D.green
    }},
      React.createElement('span', null, '$0.24'),
      React.createElement('span', { style:{color: D.text_dim, fontSize: Math.round(width * 0.040)} }, '|'),
      React.createElement('span', null, '11s'),
      React.createElement('span', { style:{color: D.text_dim, fontSize: Math.round(width * 0.040)} }, '|'),
      React.createElement('span', { style:{ fontSize: Math.round(width * 0.040)} }, 'VERIFIED')
    )
  ]
});
