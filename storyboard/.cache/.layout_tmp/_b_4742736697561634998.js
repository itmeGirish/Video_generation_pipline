...
const leafDefs = [
  {x:Math.round(w*0.04), col:D.green, q:'Answer obvious from input?', a:'YES â†’ LOW'},
  {x:Math.round(w*0.36), col:D.amber, q:'Multiple reasoning steps?', a:'YES â†’ MEDIUM'},
  {x:Math.round(w*0.68), col:D.red, q:'Wrong answer costly?', a:'YES â†’ HIGH / MAX'},
];

// Pulse: scale 1 â†’ 1.04 â†’ 1 over 12 frames
const pulse = 1 + 0.04 * Math.sin(Math.PI * interpolate(frame, [0, 12], [0, 1], {extrapolateRight:'clamp'}));
...
const banner = React.createElement('div', {style:{
  ...
  width:w, height:bannerH, backgroundColor:D.cyan,
  display:'flex', alignItems:'center', justifyContent:'center',
}},
  React.createElement('div', {style:{
    color:D.bg, fontFamily:D.font_display, fontWeight:'700',
    fontSize:Math.round(w*0.015), textAlign:'center',
  }}, 'OPENAI DEFAULT: MEDIUM â€” BENCHMARK DOWN FOR SIMPLE TASKS.')
);
