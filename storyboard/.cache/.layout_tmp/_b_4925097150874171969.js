...
function impact(emoji, label, body){
  return React.createElement('div', {
    style:{ display:'flex', flexDirection:'column', gap: 10,
            padding: 28, border: '3px solid ' + D.red, borderRadius: 12,
            backgroundColor: D.surface, width: '30%' }
  },
    React.createElement('div', { style:{ fontSize: Math.round(width * 0.040) } }, emoji),
    _scnTitle(label, D.red, Math.round(width * 0.024)),
    _scnSubtitle(body, D.text)
  );
}
const __mainEl = React.createElement('div', {
  ...
  impact('$', 'MONEY', 'Wrong financial citation → liability'),
  impact('!', 'REPUTATION', 'Fabricated source → credibility loss'),
  impact('@', 'JOB', 'Made-up legal precedent → sanctions')
);
