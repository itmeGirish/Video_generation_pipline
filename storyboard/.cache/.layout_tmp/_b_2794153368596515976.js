...
const __mainEl = _scnCard({ borderColor: D.amber, fadeIn,
  width: Math.round(width * 0.80), height: Math.round(height * 0.30),
  top: Math.round(height * 0.35),
  children: [
    _scnRow([
      React.createElement('span', { style:{ fontFamily: D.font_mono, fontWeight: 900,
                                              fontSize: Math.round(width * 0.060), color: D.amber,
                                              letterSpacing: 4 }}, '03'),
      React.createElement('span', { style:{ width: 4, height: 64, backgroundColor: D.amber }}),
      _scnTitle('BUILD PIPELINE', D.text, Math.round(width * 0.034)),
    ], 24),
    _scnSubtitle('Code in description. Start with one high-stakes task', D.text_dim, Math.round(width * 0.018)),
  ]
});
