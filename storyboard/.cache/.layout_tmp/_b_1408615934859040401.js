...
function stage(label, color, body){
  return React.createElement('div', {
    style:{ display:'flex', flexDirection:'column', alignItems:'center', gap: 8,
            padding: 24, border: '3px solid ' + color, borderRadius: 12,
            backgroundColor: D.surface, minWidth: '22%' }
  },
    _scnSubtitle(label, color),
    React.createElement('div', { style:{ fontSize: Math.round(width * 0.04), color } }, body)
  );
}
const __mainEl = React.createElement('div', {
  ...
  stage('USER QUERY', D.cyan, '●'),
  React.createElement('span', { style:{ color: D.amber, fontSize: 36 } }, '→'),
  stage('GPT-5.5', D.violet, 'DRAFT'),
  React.createElement('span', { style:{ color: D.amber, fontSize: 36 } }, '→'),
  stage('CLAUDE 4.7', D.cyan, 'VERIFY'),
  React.createElement('span', { style:{ color: D.amber, fontSize: 36 } }, '→'),
  stage('DONE', D.amber, '✓')
);
