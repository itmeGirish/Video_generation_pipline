...
const cards = cfgs.map(function(cfg, i) {
  ...
  const levelEls = cfg.levels.map(function(lv) {
    ...
    return React.createElement('div', {
      ...
      style: {
        padding: '4px 10px',
        color: isHigh ? D.red : D.text_dim,
        border: '1px solid ' + (isHigh ? D.red : D.surface),
        borderRadius: 4,
        ...
        width: '78%',
        textAlign: 'center',
        boxShadow: isHigh ? ('0 0 10px ' + D.red + '55') : 'none',
      }
    }, lv);
  });
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', {style:{
      color: D.red, fontFamily: D.font_mono, fontSize: Math.round(w * 0.010),
