...
// Simple chips already placed (static)
...
const simpleEls = simpleChips.map(function(chip, i) {
  ...
  return React.createElement('div', {key:chip, style:{
    position:'absolute', left:chipX, top:chipY, width:chipW, height:chipH,
    backgroundColor: D.surface, border:'1px solid ' + D.green, borderRadius:4,
...
const lowBadge = React.createElement('div', {style:{
  ...
  top: compartTop + compartH - Math.round(h * 0.085),
...
const complexEls = complexChips.map(function(chip, i) {
  ...
  const chipH = Math.round(h * 0.058);
  const chipW = Math.round(compartW * 0.85);
  const chipX = cCompX + Math.round((compartW - chipW) / 2);
  const chipY = compartTop + Math.round(h * 0.09) + i * (chipH + Math.round(h * 0.010));
  return React.createElement('div', {key:chip, style:{
    position:'absolute', left:chipX, top:chipY, width:chipW, height:chipH,
    backgroundColor: D.surface, border:'1px solid ' + D.amber, borderRadius:4,
    display:'flex', alignItems:'center', justifyContent:'center',
...
const spMed = spring({frame: frame - 24, fps, config:{damping:20, stiffness:200}});
const medBadge = React.createElement('div', {style:{
  position:'absolute',
  left: cCompX + Math.round(compartW * 0.10),
  top: compartTop + compartH - Math.round(h * 0.085),
  width: Math.round(compartW * 0.80),
  textAlign:'center', color: D.amber, fontFamily: D.font_display, fontWeight:'700',
  fontSize: Math.round(w * 0.014),
  opacity: spMed, transform: 'scale(' + (0.6 + 0.4 * spMed) + ')',
}}, 'MEDIUM EFFORT');

return compartEls.concat(simpleEls).concat([lowBadge]).concat(complexEls).concat([medBadge]);
