...
const __mainEl = _scnCard({ borderColor: D.amber, fadeIn,
  children: [
    _scnTitle('USE THIS FOR:', D.amber, Math.round(width * 0.022)),
    _scnLine('Deciding hallucination tolerance for your use case.', D.text),
    _scnLine('HIGH STAKES (code, legal, medical, finance) → Claude or pipeline', D.cyan),
    _scnLine('LOW STAKES (drafts, ideation, automation) → GPT-5.5 fine', D.violet),
    _scnLine('RULE: If verification cost > query cost × 4, use pipeline.', D.amber),
  ]
});
