...
const __mainEl = _scnCard({ borderColor: D.amber, fadeIn,
  ...
  children: [
    _scnTitle('USE THIS FOR EVERY PROMPT', D.amber, Math.round(width * 0.026)),
    _scnLine('  Bug fix in prod code → YES + YES → Claude', D.cyan),
    _scnLine('  Brainstorm names → NO + NO → GPT-5.5', D.violet),
    _scnLine('  Auto-tag emails → NO + YES → GPT-5.5', D.violet),
    _scnLine("  Draft contract → YES + NO → Either (you'll review)", D.amber),
  ]
});
