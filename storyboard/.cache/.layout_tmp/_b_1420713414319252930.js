...
const __mainEl = _scnCard({ borderColor: D.violet, fadeIn,
  children: [
    _scnTitle('OPENAI — THE PLATFORM', D.violet, Math.round(width * 0.030)),
    _scnRow([
      _scnPill('900M users', D.violet),
      _scnPill('50M subscribers', D.violet),
      _scnPill('$852B', D.amber),
      _scnPill('$30B ARR', D.amber),
    ]),
    _scnRow([
      _scnPill('ChatGPT', D.white),
      _scnPill('Codex', D.white),
      _scnPill('Atlas', D.white),
      _scnPill('Agents', D.white),
    ]),
    _scnSubtitle('Competing with Google Search.', D.text_dim),
  ]
});
