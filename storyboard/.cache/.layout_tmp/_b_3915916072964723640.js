...
return _scnCard({ borderColor: D.violet, fadeIn,
  width: Math.round(width * 0.50), height: Math.round(height * 0.62),
  top: Math.round(height * 0.18), padding: 48,
  children: [
    _scnTitle("GPT-5.5 'SPUD'", D.violet, Math.round(width * 0.030)),
    _scnSubtitle('RELEASED APR 23, 2026', D.violet),
    _scnLine('BUILT FOR ACTION', D.text, Math.round(width * 0.024), 900),
    React.createElement('div', { style:{ marginTop: 12 } },
      _scnRow([
        _scnPill('TOOL ORCHESTRATION', D.violet),
        _scnPill('BROWSER + DESKTOP', D.violet),
        _scnPill('FAST', D.green),
      ])
    )
  ]
});
