...
const __mainEl = _scnCard({ borderColor: D.amber, fadeIn,
  width: Math.round(width * 0.70), height: Math.round(height * 0.30),
  top: Math.round(height * 0.35),
  children: [
    _scnTitle('TWO QUESTIONS PICK YOUR AI', D.amber, Math.round(width * 0.034)),
    _scnSubtitle('1. Does accuracy matter?    2. Will it run unsupervised?', D.text)
  ]
});
