...
### Pair-block structure (PREFERRED) — narration coupled into each bullet

```
# Video Title — PRODUCTION DOCUMENT

## SCENE 1 — "Title" (0:00 – M:SS)           ← ## SCENE header
- **0:00 – 0:06 — [REPLACE] Headline.**      ← bullet, NO ### Animation header
  > Narration for this beat. <pause 0.3s>    ← > line(s) inside the bullet
  Visual description...                       ← non-`>` lines = the visual
  audio_anchor: phrase from this beat's `>`   ← verbatim substring of the line above
- **0:06 – 0:12 — Next headline.**
  > Next narration. <pause 0.5s>
  Visual...
## SCENE 2 — "Title" (M:SS – M:SS)
- **...**
```

Pair-block detection + rules (matches `source_parser.py` `_extract_pair_blocks`):
- A scene is pair-block when it has **NO `### Animation` and NO `### Narration`** header —
  the parser reassembles narration from the `>` lines inside the bullets, in order.
- Do NOT mix: a scene is either legacy (both headers) or pair-block (neither). One scene
  with `### Narration` but no `### Animation` (or vice-versa) is a FAIL.
- Every pair-block bullet should have at least one `>` narration line (a purely-visual
  sub-beat may omit it, but then its `audio_anchor` must reference an earlier beat's line).
- `audio_anchor:` value must be a verbatim substring of that bullet's own `>` line.
- Optional `anchor_mode:` line — one of `appear` (default) / `through` / `land` (sync-to-
  meaning, see `script-animation-bullets`). If present it must be exactly one of those three
  values; any other value is a FAIL. Omitted = `appear`.
...
- Full format spec with worked examples: `.claude/skills/script_generation/references/script_formats.md`
- Fireship-style pacing/voice/density layer: `.claude/skills/script_generation/references/fireship_format.md`
