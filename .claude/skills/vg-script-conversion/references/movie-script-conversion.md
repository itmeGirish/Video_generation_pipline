# Rich / Movie-Script Conversion (reference for `vg-script-conversion` — the MOVIE-SCRIPT mode)

> This is the detailed checklist for converting the **movie-script format** (frame timing · `### VO:`/`### ANIMATION:`
> headers · sub-scenes · design-token preambles · bespoke metaphors) → canonical. It was formerly the standalone
> `vg-rich-script-conversion` skill; it is now a reference of the single converter owner **`vg-script-conversion`**
> (invoke that skill; this file is its movie-script mode). The canonical target format is owned by `vg-source-script-format`.
