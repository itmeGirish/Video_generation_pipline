...
description: How per-bullet React.createElement code is authored in-session and seeded into the cache. NO subprocess to claude CLI — this rule supersedes the previous LLM-codegen design.
metadata:
  tags: visual-designer, in-session, authoring, cache, blocks, fidelity
---
...
EXACTLY (`prompt-v13-per-bullet` constant) so the cache file lands at the path
