...
description: How the .claude/script_generation skill (YouTube scriptwriter) feeds into the video_generation pipeline. Two-step bridge — generate, then convert — with no changes to the existing parser/build code.
metadata:
  tags: script-generation, integration, entrypoint, scriptwriter, bridge
---
