---
name: rich-script-conversion
description: MANDATORY first step when a raw script is given. Read the raw `projects/scripts/<name>.txt`, convert to canonical format using the 9-step checklist, and write the result to `projects/structured_scripts/<name>.txt`. Handles frame-based timing, ### VO:/### ANIMATION: headers, bespoke cinematography, sub-scenes, design-token preambles — all formats `script_converter.py` does NOT yet handle.
metadata:
  tags: conversion, rich-script, bespoke-animation, structure, format, mandatory-first-step
---
