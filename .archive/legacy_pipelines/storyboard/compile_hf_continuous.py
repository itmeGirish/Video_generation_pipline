...
2. Generate ONE TTS audio file (vo-harness3-full.mp3) so the narrator's prosody
...
SCENES = [f"hf{i:02d}" for i in range(1, 8)]
SCENES_DIR = ROOT / "storyboard" / "scenes"
AUDIO_DIR = ROOT / "projects" / "harness_3" / "audio"
...
TIMING_OUT = ROOT / "storyboard" / "hf_timing.json"
FULL_AUDIO_NAME = "vo-harness3-full.mp3"
FPS = 30
...
hash_marker = AUDIO_DIR / f".vo-harness3-full.{text_hash}.hash"
...
async def generate_tts():
    ...
    # Mark cache
    for old in AUDIO_DIR.glob(".vo-harness3-full.*.hash"):
        old.unlink()
